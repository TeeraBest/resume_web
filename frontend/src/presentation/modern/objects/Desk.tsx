  ...
  return (
    <group ref={groupRef} position={position}>
      <mesh receiveShadow castShadow position={[0, 4, 0]}>
        ...
        <meshStandardMaterial color={surfaceColor} metalness={0.3} roughness={0.5} />
      </mesh>
      {/* Thin glowing rim underneath, sells the "floating" sci-fi feel */}
      <mesh position={[0, 3.72, 0]}>
        <boxGeometry args={[43.6, 0.04, 33.6]} />
        <meshStandardMaterial color={rimColor} emissive={rimEmissive} emissiveIntensity={0.6} toneMapped={false} />
      </mesh>
      {children}
    </group>
  )
